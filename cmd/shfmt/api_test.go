...
func TestAPI_CoversExports(t *testing.T) {
	...
	if info.ConfigKey != "go-shfmt" {
		t.Fatalf("plugin config key = %q; want %q", info.ConfigKey, "go-shfmt")
	}
	...
	if resolved.Indent != 2 {
		t.Fatalf("resolved indent = %d; want 2", resolved.Indent)
	}
	...
	if err != nil {
		t.Fatalf("formatShell failed: %v", err)
	}
	loadShared(bad)
	...
	if !bytes.Equal(shared[:outSize], want) {
		t.Fatalf("formatted output did not match shfmt")
	}

	badErr := []byte("if")
	if _, parseErr := formatShell(badErr, defaultConfig()); parseErr == nil {
		t.Fatalf("expected formatShell error for invalid input")
	}
	loadShared(badErr)
