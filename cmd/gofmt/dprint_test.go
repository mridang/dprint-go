...
//
// The test builds the WebAssembly plugin using the local Makefile
// target, writes a deliberately malformed Go file in a temporary
// directory, invokes `dprint fmt` with the repo configuration, and
// asserts the resulting file bytes match the canonical output from
// go/format. It then runs dprint a second time to assert idempotence.
//
// Preconditions:
//
