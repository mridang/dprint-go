...
// writes a deliberately malformed Terraform file in a temporary directory,
// invokes `dprint fmt` with the repo configuration, and asserts the resulting
// file bytes match the canonical output from hclwrite. It then runs dprint a
// second time to assert idempotence.
//
// Preconditions:
//
